...
import {
  GainersDataType,
  LosersDataType,
  SectorType, TopIndustriesDataType,
...
const useGainers = (period: number) => {
  ...
  const {isPending, error, data} = useQuery<GainersDataType>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/dashboard/gainers?period=${encodeURIComponent(period)}&countries=${encodeURIComponent(JSON.stringify(countries))}`);
...
const useLosers = (period: number) => {
  ...
  const {isPending, error, data} = useQuery<LosersDataType>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/dashboard/losers?period=${encodeURIComponent(period)}&countries=${encodeURIComponent(JSON.stringify(countries))}`);
      return await response.json();
    }
  });
...
const useTopIndustries = (period: number) => {
  ...
  const {isPending, error, data} = useQuery<TopIndustriesDataType>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/dashboard/top-industries?period=${encodeURIComponent(period)}&countries=${encodeURIComponent(JSON.stringify(countries))}`);
      return await response.json();
    }
  });
...
const useWorstIndustries = (period: number) => {
  ...
  const {isPending, error, data} = useQuery<WorstIndustriesDataType>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/dashboard/worst-industries?period=${encodeURIComponent(period)}&countries=${encodeURIComponent(JSON.stringify(countries))}`);
      return await response.json();
    }
  });
