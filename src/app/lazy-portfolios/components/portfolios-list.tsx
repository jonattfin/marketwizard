...
const useLazyPortfolios = (pageNumber: number, pageSize: number) => {
  const {isPending, error, data} = useQuery<[number, LazyPortfolioType[]]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/lazy-portfolios/all?pageNumber=${encodeURIComponent(pageNumber)}&pageSize=${pageSize}`);
    ...
    <>
      <Breadcrumb.Root>
        <Breadcrumb.List>
          ...
          <Breadcrumb.Item>
            <Breadcrumb.CurrentLink data-testid={"lazy-portfolio-link"}>Lazy Portfolios</Breadcrumb.CurrentLink>
          </Breadcrumb.Item>
        </Breadcrumb.List>
      </Breadcrumb.Root>
      <div>&nbsp;</div>
      ...
      <Text textStyle={"m"} color={"gray.500"}>
        ...
        It’s called lazy because you don’t actively manage your portfolio.
        It’s the so called buy and hold investing strategy, designed to achieve a long-term financial independence.
        That means no active trading, no checking your stocks every day, and no paying some hedge fund manager (who
        won’t beat the market anyway) to handle your money.
      </Text>
      <div>&nbsp;</div>
      ...
      <Center>
        <Pagination.Root
          count={numberOfLazyPortfolios} pageSize={pageSize} page={pageNumber}
