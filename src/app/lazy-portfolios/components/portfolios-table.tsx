import { Center, FormatNumber, Stack, Table, Tag } from "@chakra-ui/react";
          ...
          <Table.Body>
            {items.map((item) => (
              <Table.Row key={item.id}>
                <Table.Cell>
                  <Link href={"/lazy-portfolios/" + item.id}>{item.name}</Link>
                </Table.Cell>
                <Table.Cell textAlign="end">
                  <Tag.Root
                    size="md"
                    variant={"subtle"}
                    colorPalette={item["1d"] > 0 ? "green" : "orange"}
                  >
                    <Tag.Label>
                      <FormatNumber
                        value={item["1d"]}
                ...
                <Table.Cell textAlign="end">
                  <Tag.Root
                    size="md"
                    variant={"subtle"}
                    colorPalette={item.ytd > 0 ? "green" : "orange"}
                  >
                    <Tag.Label>
                      <FormatNumber
                        value={item.ytd}
